// scripts/import-communication-vocab.ts
// ============================================================================
// HƯỚNG DẪN SỬ DỤNG:
//
//   Chạy thử 10 từ đầu (test):
//     npx tsx scripts/import-communication-vocab.ts
//
//   Import toàn bộ từ vựng Giao tiếp:
//     npx tsx scripts/import-communication-vocab.ts --all
//
//   XÓA SẠCH từ vựng Giao tiếp cũ và import lại toàn bộ:
//     npx tsx scripts/import-communication-vocab.ts --all --clear
//
// ============================================================================
...
async function main() {
  console.log(`\n${"=".repeat(60)}`);
  console.log(`📚 CLI IMPORT TỪ VỰNG GIAO TIẾP VÀO FIREBASE (ADMIN SDK)`);
  console.log(`   Chế độ : ${runAll ? "TOÀN BỘ" : `TEST ${TEST_LIMIT} TỪ ĐẦU (thêm --all để chạy hết)`}`);
  console.log(`   Clear  : ${clearOld ? "✅ BẬT (sẽ xóa dữ liệu Giao tiếp cũ trước khi import)" : "❌ TẮT"}`);
  ...
  if (clearOld) {
    console.log("🗑️  Đang xóa sạch dữ liệu từ vựng Giao tiếp cũ...");
    const oldDocs = await vocabRef
      .where("courseId", "in", ["tu-vung-giao-tiep"])
      .get();
    ...
    console.log(`   ✅ Đã xóa thành công ${totalDeleted} tài liệu Giao tiếp cũ.`);
  }
  ...
  for (let i = 0; i < dataRows.length; i++) {
    ...
    let example = "";
    let exampleMeaning = "";
    let courseId = "tu-vung-giao-tiep";
    let lessonId = "lesson-01";
    let lessonTitle = "Bài học Giao tiếp";
    let courseName = "Từ vựng Giao tiếp";
    ...
    if (commaHeader) {
      ...
      level = String(row.level || row.Level || "N5").trim();
      ...
      courseId = String(row.courseId || row.CourseId || "tu-vung-giao-tiep").trim();
      lessonId = String(row.lessonId || row.LessonId || "lesson-01").trim();
      lessonTitle = String(row.lessonTitle || row.LessonTitle || "Bài học Giao tiếp").trim();
    }
    ...
    try {
      ...
      const docId = `COMM_${lessonId}_${cleanWord}`;
  ...
  if (!runAll) {
    ...
    console.log(`   npx tsx scripts/import-communication-vocab.ts --all --clear`);
  }
  console.log(`${"=".repeat(60)}\n`);
  process.exit(0);
}
